...
@interface firstXiangQingTableViewCell ()
@property(nonatomic,strong)TQRichTextView *titleLabel;
...
-(void)setMainView{
    ...
    self.titleLabel = [[TQRichTextView alloc]init];
    self.titleLabel.frame = CGRectMake(10, 10, 190, 0);
    self.titleLabel.textColor = [UIColor colorWithHexString:@"93C6E9"];
    self.titleLabel.font = [UIFont systemFontOfSize:18];
    self.titleLabel.lineSpacing = 0.1;
...
-(void)resetViewView:(NSDictionary *)dic{
    ...
    if ([photoArray isKindOfClass:[NSArray class]]) {
        self.photoImage.frame = CGRectMake(7.5, 10, 200, ((photoArray.count + 1)/2) *100);
        ...
        self.photoImage.frame = CGRectMake(10, 0, 200, 0);
    }

    self.titleLabel.text = [NSString stringWithFormat:@"[p1000]%@",[DictionaryStringTool stringFromDictionary:dic forKey:@"storyPlace"]];

    if (self.titleLabel.text.length > 13) {
        self.titleLabel.frame = CGRectMake(10, self.photoImage.frame.size.height + self.photoImage.frame.origin.y + 10, 190, self.titleLabel.drawheigth);
    }
    else{
        self.titleLabel.frame = CGRectMake(10, self.photoImage.frame.size.height + self.photoImage.frame.origin.y + 0, 200, 0);
    }
    
    self.xiangQingLabel.text = [DictionaryStringTool stringFromDictionary:dic forKey:@"storyDesc"];
    if(self.xiangQingLabel.text.length > 0){
        self.xiangQingLabel.frame = CGRectMake(10, self.titleLabel.frame.size.height + self.titleLabel.frame.origin.y + 10, 190, self.xiangQingLabel.drawheigth);
    }
    else{
        self.xiangQingLabel.frame = CGRectMake(0, self.titleLabel.frame.size.height + self.titleLabel.frame.origin.y, 0, 0);
    }
