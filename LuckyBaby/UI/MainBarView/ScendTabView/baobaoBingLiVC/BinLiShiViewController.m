...
@interface BinLiShiViewController ()<UITableViewDataSource,UITableViewDelegate>
@property(nonatomic,strong)UITableView *_tableView;
@property(nonatomic,strong)NSArray *benliShiArray;

@end

@implementation BinLiShiViewController

-(void)viewWillAppear:(BOOL)animated{
    [self getData];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(listDiseaseSumSuccess:) name:@"listDiseaseSumSuccess" object:nil];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(listDiseaseSumFail:) name:@"listDiseaseSumFail" object:nil];
}
...
-(void)getData{
    self.benliShiArray = [NSArray array];
    jianKangNetWork *jianK = [[jianKangNetWork alloc]init];
    NSDictionary *dic = [plistDataManager getDataWithKey:user_loginList];
    [jianK listDiseaseSumWithChildIdFamily:[DictionaryStringTool stringFromDictionary:dic forKey:@"childIdFamilyCurrent"]];
}

#pragma mark -logic data
-(void)listDiseaseSumSuccess:(NSNotification*)noti{
    NSDictionary *dic = [noti object];
    NSArray *array = [dic objectForKey:@"data"];
    if ([array isKindOfClass:[NSArray class]]) {
        self.benliShiArray = array;
    }
    [self._tableView reloadData];
}

-(void)listDiseaseSumFail:(NSNotification*)noti{
...
-(void)setTableView{
    ...
    HJHMyLabel *nameLabel = [[HJHMyLabel alloc]initWithFrame:CGRectMake(0, 0, 100, 35)];
    nameLabel.text = @"病名";
...
{
    return self.benliShiArray.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *dic = [self.benliShiArray objectAtIndex:indexPath.row];
    TQRichTextView *xiangQingLabel = [[TQRichTextView alloc]initWithFrame:CGRectMake(0, 0, 100, 0)];
    xiangQingLabel.text = [DictionaryStringTool stringFromDictionary:dic forKey:@"diseaseType"];
