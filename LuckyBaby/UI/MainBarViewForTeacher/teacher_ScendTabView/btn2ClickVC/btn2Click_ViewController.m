...
@interface btn2Click_ViewController ()<UITextFieldDelegate,UITextViewDelegate,addClassViewDelegate,HJHPickerViewDelegate>
...
@implementation btn2Click_ViewController
...
-(void)rightClickBtnClick{
    addClassViewController *aVC = [[addClassViewController alloc]initWithBuMenArray:self.fisrtXiangQingArray];
    aVC.delegate2 = self;
...
-(void)selectBuMenArray:(NSArray *)array{
    ...
    for (int i = 0; i < self.buMenArray.count; i++) {
        ...
        if (i == 0) {
            self.textField1.text = [DictionaryStringTool stringFromDictionary:dic forKey:@"className"];
        }
        else{
            self.textField1.text = [NSString stringWithFormat:@"%@,%@",self.textField1.text,[DictionaryStringTool stringFromDictionary:dic forKey:@"className"]];
        }
...
-(void)saveBtnClick{
    ...
    if (self.textView.text.length > 0 && self.textField1.text.length > 0 && self.textField2.text.length > 0 && self.textField3.text.length > 0 &&self.textField4.text.length > 0) {
        ...
        [tNet savePlatformInformWithGradeWithInfoTitle:self.textField3.text infoContent:self.textView.text dateStart:time1 dateEnd:time2 semesterId:[DictionaryStringTool stringFromDictionary:dic forKey:@"semesterId"] publicClass:@"" publicClassNames:@"" publicDept:string1 publicDeptNames:string2 platformId:[DictionaryStringTool stringFromDictionary:dic forKey:@"platformId"]];
    }
    else{
