...
/*!
 ...
 @discussion 当EMConversation对象的enableReceiveMessage属性为YES时, 会触发此回调
             ...
             下载完所有附件后, 回调didFetchMessageAttachmentsFinished:error:会被触发
 */
...
/*!
 ...
 @discussion SDK接收到消息时, 有以下两种情况:
    1. 如果是带缩略图的消息时(图片或Video), 会自动下载缩略图, 
        下载完成后, 会调用 didFetchMessageAttachmentsFinished:error这个回调
    2. 如果是语音消息时, 会自动下载语音附件, 
        会调用 didFetchMessageAttachmentsFinished:error这个回调
