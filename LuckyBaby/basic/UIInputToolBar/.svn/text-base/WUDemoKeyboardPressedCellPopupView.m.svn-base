...
@property (nonatomic,weak) UIImageView *popupBackground;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    //2014.01.17 这里控制的是点击表情时候的动画效果
        ...
        self.popupBackground = popupBackgroundView;
...
- (void)setKeyItem:(WUEmoticonsKeyboardKeyItem *)keyItem {
    _keyItem = keyItem;
    if (![keyItem.textToInput isEqualToString:@"Expression_close"]) {
        UIImage *popupBackground = [UIImage imageNamed:@"keyboard_popup"];
        self.popupBackground.image = popupBackground;
        self.textLabel.text = keyItem.textToInput;
        self.imageView.image = keyItem.image;
    }else{
        self.textLabel.text = Nil;
        self.imageView.image = Nil;
        self.popupBackground.image = Nil;
    }
}
