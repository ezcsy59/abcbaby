...
+ (WUEmoticonsKeyboard *)sharedEmoticonsKeyboard {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int i = 0; i<111; i++) {
            if (i == 34 || i == 69 || i == 104 || i == 110) {
                WUEmoticonsKeyboardKeyItem *loveKey = [[WUEmoticonsKeyboardKeyItem alloc] init];
                loveKey.image = [UIImage imageNamed:@"Expression_close.png"];
                loveKey.textToInput = @"Expression_close";
                [itemsArray addObject:loveKey];
                
            }else{
                WUEmoticonsKeyboardKeyItem *loveKey = [[WUEmoticonsKeyboardKeyItem alloc] init];
                loveKey.image = [UIImage imageNamed:[NSString stringWithFormat:@"[p%d].png",i]];
                loveKey.textToInput = [NSString stringWithFormat:@"[p%d]",i];
                [itemsArray addObject:loveKey];
            }
        }
        //Icon key group
        WUEmoticonsKeyboardKeyItemGroup *imageIconsGroup = [[WUEmoticonsKeyboardKeyItemGroup alloc] init];
        imageIconsGroup.keyItems = itemsArray;
...
//        [keyboard setImage:[UIImage imageNamed:@"cancle.png"] forButton:WUEmoticonsKeyboardButtonBackspace state:UIControlStateNormal];
...
//        [keyboard setImage:[UIImage imageNamed:@"cancle.png"] forButton:WUEmoticonsKeyboardButtonBackspace state:UIControlStateHighlighted];
        //Keyboard background
