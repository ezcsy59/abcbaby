...
- (void)start
{
  ...
  //  NSString* versionString = [[[[AppDevice stringByAppendingString:@"/"]stringByAppendingString:AppVersion]stringByAppendingString:@"/"]stringByAppendingString:AppPublishMode];
   
    
    
       
    
   // NSDictionary* requestHeader = @{ @"X-User-Key" : [NSString stringWithFormat:@"%lld",[DataCenter sharedInstance].getCurrentUser.userId], @"X-Session-Key" : [NSString stringWithFormat:@"%@",[DataCenter sharedInstance].getCurrentUser.sessionId],@"X-API-Key":versionString};
  //  NSLog(@"dict%@",requestHeader);
    
  //  for (id key in requestHeader) {
  //      [request setValue:[requestHeader objectForKey:key] forHTTPHeaderField:key];
  //  }
